        ...
        if ( messageType == "eyes" ){
        	...
        	if ( agentEvent == "artifactSeen" ) {
         		...
         		sightData = new SightData( "artifact", art_info.ArtifactType, art_info.ArtifactName );
          		...
          		sightData = new SightData( "agent", null, param );
          }
          	wsMsg = new BrainMessage( "body", "vesna", "sight", sightData );
        }
        // SignalData signalData = new SignalData( messageType, agentEvent, messagePayload );
        // BrainMessage wsMsg = new BrainMessage( "body", agentName, "signal", signalData);
